...
#define SET_BIT_DEFINE(pos) ((uint8_t) (1<<( (uint8_t) (pos) )))
...
{
  hal_nrf_write_reg(STATUS, SET_BIT_DEFINE(int_source));
}

bool hal_nrf_get_irq_mode(uint8_t int_type)
{
  if(hal_nrf_read_reg(CONFIG) & SET_BIT_DEFINE(int_type))
    return false;
  else
    return true;
}
...
void hal_nrf_lock_unlock()
{
  ...
  /*NRF24_CSN_LOW();
  SPI_send_one_byte(nrf24_SPIx, LOCK_UNLOCK);
  SPI_send_one_byte(nrf24_SPIx, 0x73);
  NRF24_CSN_HIGH();*/
